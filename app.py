...
with tab_metrics:
    ...
    if st.button("Compute Aggregate Metrics"):
        ...
        col4.metric("Overall Accuracy", f"{round(acc*100,2)}%")
        ...
        if categories:
            ...
            ax2.bar(categories, counts, color=['purple', 'orange', 'red', 'green', 'blue'])
...
with tab_curve:
    ...
    if st.button("Plot Accuracy Curve"):
        ...
        ax.plot(th, acc_values, marker='o', linestyle='-', label='Accuracy', color='blue')
        ax.plot(th, recall_values, marker='x', linestyle='--', label='Recall', color='green')
        ax.plot(th, fpr_values, marker='s', linestyle='-.', label='False Positive Rate', color='red')
